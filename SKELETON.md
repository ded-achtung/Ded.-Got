...
│   └── wpe-cli/                # минимальный CLI skeleton (`diag`)
├── deny.toml
├── clippy.toml
├── tools/
...
Сознательно **не в skeleton**: `wpe-wayland`, `wpe-render-wgpu`, `wpe-backend-*`, `wpe-daemon`, `wpe-gui`, `wpe-profile`, `wpe-ipc`. Они появляются после skeleton stable.
...
pub trait FrameSource: Send {
    ...
    fn resize(&mut self, size: SurfaceSize, scale: FractionalScale)
        -> Result<(), BackendError>;
    fn render_frame(&mut self, req: &FrameRequest)
...
Контракт синхронизирован с прототипом: единая точка `render_frame`, без `update + produce`.
...
- `prepare` вызывается до `render_frame`/`resize` — падение иначе.
- `resize` не вызывается во время `render_frame` (reentrancy guard).
- после `prepare` вызовы `render_frame` и `resize` работают в нормальном порядке.
...
fn lifecycle_order() {
    ...
    // render_frame до prepare должен вызвать guard panic.
    let req = FrameRequest {
        target_size: SurfaceSize { width: 1920, height: 1080 },
        is_first_frame: true,
    ...
    assert!(result.is_err(), "render_frame before prepare must fail");

    // После prepare — ок.
    let ctx = TracedBackend::mock_prepare_ctx();
    let _ = backend.prepare(ctx);
    let ok = backend.render_frame(&req);
    assert!(ok.is_ok());

    // resize после prepare и между кадрами — ок.
    let resized = backend.resize(
...
- [ ] `cargo test --workspace` зелёный, все 5 contract-tests (16 sub-tests).
- [ ] `forbidden-imports.sh` exit 0.
- [ ] `cargo clippy -- -D warnings` зелёный.
- [ ] `cargo deny check` зелёный.

**Quality:**
- [ ] 5 contract-tests реализованы (lifecycle_order, pause_idempotent, present_recovery, no_gpu_in_backend, frame_output_invariants). Отсутствующий — не merge.
...
1. **Cargo workspace** с пятью crate'ами (включая `wpe-cli` skeleton). CI с `cargo build` (пустые crate'ы компилируются).
